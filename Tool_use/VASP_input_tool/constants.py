...
FUNCTIONAL_INCAR_PATCHES: Dict[str, Dict[str, Any]] = {
    ...
    # BEEFVTST: identical INCAR to BEEF; the PBS script TYPE1 label
    # ("beefvtst") selects the VTST-patched VASP binary for NEB/DIMER.
    # Not a separate DFT functional — user-facing functionals are PBE/BEEF/SCAN/PBE0/HSE.
    # BEEFVTST：INCAR 与 BEEF 完全相同；TYPE1="beefvtst" 选择带 VTST 补丁的
    # VASP 可执行文件（过渡态搜索专用）。并非独立 DFT 泛函。
    "BEEFVTST": {
        "GGA": "BF",
        "LUSE_VDW": True,
        "AGGAC": 0.0000,
