...
from .calc_registry import CALC_TYPE_TO_CATEGORY, VDW_FUNCTIONALS, get_template_name
...
# Template selection and VDW_FUNCTIONALS are now in calc_registry.py.
# get_template_name() and VDW_FUNCTIONALS are imported above.
...
class ScriptWriter:
    ...
    def _resolve_template(self, calc_type: str) -> Path:
        return SCRIPT_DIR / get_template_name(calc_type)
    ...
    def _copy_vdw_kernel(self, functional: str, output_dir: Path) -> None:
        if functional.upper() not in VDW_FUNCTIONALS:
            return
